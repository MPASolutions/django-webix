...
class TestAdminWebixCalls(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username="testuser1", password="12345", is_staff=True, is_superuser=True, is_active=True
        )
    ...
    def test_user_group_modeladmin_list(self):
        self.client.login(username="testuser1", password="12345")
        User = get_user_model()
        _url = reverse("dwadmin:{}.{}.list".format(User._meta.app_label, User._meta.model_name))
        response = self.client.get(_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_group_update(self):
        ...
        User = get_user_model()
        _url = reverse(
            "dwadmin:{}.{}.update".format(User._meta.app_label, User._meta.model_name), kwargs={"pk": self.group1.pk}
        )
        response = self.client.get(_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_menu_list(self):
        self.client.login(username="testuser1", password="12345")
        _url = reverse(
            "admin:{}_{}_changelist".format(WebixAdminMenu._meta.app_label, WebixAdminMenu._meta.model_name)
        )
        response = self.client.get(_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_menu_create(self):
        self.client.login(username="testuser1", password="12345")
        _url = reverse("admin:{}_{}_add".format(WebixAdminMenu._meta.app_label, WebixAdminMenu._meta.model_name))
        response = self.client.get(_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_modeladmin_create(self):
        self.client.login(username="testuser1", password="12345")
        User = get_user_model()
        _url = reverse("dwadmin:{}.{}.create".format(User._meta.app_label, User._meta.model_name))
        response = self.client.post(
            _url,
            {
                "username": "testttt",
    ...
    def test_user_modeladmin_update(self):
        ...
        _url = reverse(
            "dwadmin:{}.{}.update".format(User._meta.app_label, User._meta.model_name), kwargs={"pk": self.user1.pk}
        )
        response = self.client.post(
            _url,
            {
