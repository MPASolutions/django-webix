...
from django_webix.admin_webix.admin_webix import UserAdmin, GroupAdmin
...
class TestAdminWebixCalls(TestCase):
    ...
    def test_modeladmin_list(self):
        self.client.login(username='testuser1',
                          password='12345')
        _url = reverse('admin_webix:app_name.mymodel.list')
    ...
    def test_user_group_modeladmin_list(self):
        self.client.login(username='testuser1',
                          password='12345')
        User = get_user_model()
        _url = reverse('admin_webix:{}.{}.list'.format(User._meta.app_label,
    ...
    def test_user_group_update(self):
        self.client.login(username='testuser1',
                          password='12345')
        User = get_user_model()
        _url = reverse('admin_webix:{}.{}.update'.format(User._meta.app_label,
    ...
    def test_user_modeladmin_passwordchange(self):
        self.client.login(username='testuser1',
                          password='12345')
        _url = reverse('admin_webix:admin_webix_password_change_admin', kwargs={'pk':self.user1.pk})
        response = self.client.post(_url, {
            'password1': '12345',
            'password2': '12345',
        })
