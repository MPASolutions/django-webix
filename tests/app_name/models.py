...
class InlineStackedModel(models.Model):
    ...
    choicefield = models.CharField(
        max_length=255, blank=True, null=True, choices=(("test", "Test"), ("sample", "Sample")), default="test"
    )
    inlinemodel = models.ForeignKey(
        "InlineModel", blank=True, null=True, related_name="inlinemodel_fk", on_delete=models.CASCADE
    )
    inlinemodels = models.ManyToManyField("InlineModel", blank=True, related_name="inlinemodel_m2m")
    inlinemodels_default = models.ManyToManyField(
        "InlineModel", blank=True, related_name="inlinemodel_m2m_default", default=[1]
    )
...
class InlineEmptyModel(models.Model):
    ...
    choicefield = models.CharField(max_length=255, choices=(("test", "Test"), ("sample", "Sample")))
    foreign_default = models.ForeignKey("InlineStackedModel", on_delete=models.CASCADE)
    foreign_default_2 = models.ForeignKey(
        "InlineStackedModel", blank=True, null=True, related_name="inlineeptymodel_fk2", on_delete=models.CASCADE
    )
