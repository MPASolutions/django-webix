            ...
            if len(settings.LANGUAGES) > 1 and node[1] is not None:
                for abbreviation, language in settings.LANGUAGES:
                    with translation.override(abbreviation):
                        WebixAdminMenuLanguage.objects.create(
                            menu=m,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--debug', action='store_true', help='prints the menus that would be created without making the change')
