...
from django_webix.contrib.admin.forms import (
...
class LogEntryAdmin(admin.ModelWebixAdmin):
    ...
    list_display_header = {
        ...
        "url": {"field_name": "url", "queryset_exclude": True, "datalist_column": '{id: "url", hidden: true}'},
    }
    ...
    def _get_objects_datatable_values(self, view, qs):
        ...
        for item, value in zip(qs, data):
            value.update({"url": self.admin_site.get_object_url(item.content_type, object_pk=item.object_id)})
        return data
