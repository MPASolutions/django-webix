...
#         'data_type': 'int', # field type displayed: int (does not expect null, use string), string, bool, combo, date
...
class CommandParameters(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for _id, arg_p in zip(range(len(args_p)), args_p):
            par = {"field_name": arg_p.dest, "pos_id": _id}
        ...
        if hasattr(command, "help"):
            params.append({"field_name": "help", "value": command.help})
...
class CommandExecute(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        ce.last_execution_date = timezone.now()
        ce.save()
