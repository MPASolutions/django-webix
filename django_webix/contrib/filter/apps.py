...
class FilterConfig(AppConfig):
    ...
    def ready(self):
        ...
        if "visibility" in settings.DJANGO_WEBIX_FILTER and not isinstance(
            settings.DJANGO_WEBIX_FILTER["visibility"], dict
        ):
            raise ImproperlyConfigured("`visibility` must be a dict")
        if "shared_edit_groups" in settings.DJANGO_WEBIX_FILTER and not isinstance(
            settings.DJANGO_WEBIX_FILTER["shared_edit_groups"], dict
        ):
