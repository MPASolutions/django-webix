...
from django_webix.contrib.filter.utils.check import check_config, check_filter, model_is_enabled
from django_webix.contrib.filter.utils.config import _get_config_new, get_limit_suggest
from django_webix.contrib.filter.utils.json_converter import get_JSON_for_DB, get_JSON_for_JQB, get_JSON_from_DB
...
class FilterConfigView(View):
    ...
    def _get_model_config(self, model_class, already_visited, prefix=None, model_prefix=None):
        ...
        relation_fields = list(
            filter(
                lambda field: issubclass(type(field), models.ForeignKey)
                or issubclass(type(field), ForeignObjectRel)
                or issubclass(type(field), models.ManyToManyField)
                or issubclass(type(field), GenericRelation),
        ...
        for field in relation_fields:
            if issubclass(type(field), models.ForeignKey):
                ...
                results.append(
                    self._get_model_config(
                        model, already_visited=already_visited + [_model_config["model"]], prefix=field.name
                    )
                )
        ...
        if prefix is None:
            operators = list(
                set([operator for model in results for field in model["fields"] for operator in field["operators"]])
            )
            return {
                "models": results,
        ...
        if len(names) == 3:
            try:
                ...
                if isinstance(model_class._meta.get_field(field_name), ArrayField):
                    qs = qs.annotate(dw_filter_unpacked_field=Func(F(field_name), function="unnest"))
...
class WebixFilterMixin:
    ...
    def has_view_permission(self, request, obj=None):
        ...
        if obj is not None and obj.visibility == "public":
            ...
            return permission and (
                ...
                or obj.insert_user is None
                or (
                    obj.shared_edit_group is True
                    and obj.insert_user.groups.filter(pk__in=request.user.groups.values("pk")).exists()
                )
            )
            ...
            return permission and (
                obj.insert_user == request.user
                or obj.insert_user is None
                or obj.assignees_groups.filter(user=request.user).exists()
                or (
                    obj.shared_edit_group is True
                    and obj.insert_user.groups.filter(pk__in=request.user.groups.values("pk")).exists()
                )
    ...
    def has_delete_permission(self, request, obj=None):
        ...
        if obj is not None:
            return permission and (obj.insert_user == request.user or obj.insert_user is None)
        return permission
...
class WebixFilterList(WebixFilterMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.kwargs.get("app_label") and self.kwargs.get("model_name"):
            context["list_view_prefix"] = "{}_{}_".format(self.kwargs.get("app_label"), self.kwargs.get("model_name"))
        return context
    ...
    def get_initial_queryset(self):
        ...
        qs = qs.annotate(
            visibility_display=Case(
                *[
                    When(visibility=k, then=Value(force_str(v)))
                    for k, v in self.model._meta.get_field("visibility").choices
                ],
                default=Value(""),
...
class WebixFilterCreate(WebixFilterMixin, CreateView):
        ...
        if self.is_popup():
            if self.request.GET.get("model", None):
                ...
                return self.wrap_url_popup(
                    reverse(
                        "dwfilter.webixfilter.list_model", kwargs={"app_label": app_label, "model_name": module_name}
                    )
                )
    ...
    def get_initial(self):
        ...
        if "visibility" in settings.DJANGO_WEBIX_FILTER:
            ...
            group = self.request.user.groups.filter(name__in=settings.DJANGO_WEBIX_FILTER["visibility"].keys()).first()
        ...
        if "shared_edit_groups" in settings.DJANGO_WEBIX_FILTER:
            ...
            if group is not None and "default" in settings.DJANGO_WEBIX_FILTER["shared_edit_groups"][group.name]:
                initial["shared_edit_group"] = settings.DJANGO_WEBIX_FILTER["shared_edit_groups"][group.name][
                    "default"
                ]
        return initial
    ...
    def get_form_kwargs(self):
        ...
        if "visibility" in settings.DJANGO_WEBIX_FILTER:
            ...
            if (
                group is not None
                and "choices" in settings.DJANGO_WEBIX_FILTER["visibility"][group.name]
                and isinstance(settings.DJANGO_WEBIX_FILTER["visibility"][group.name]["choices"], list)
            ):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context.update({"limit_suggest": get_limit_suggest()})
        return context
...
class WebixFilterUpdate(WebixFilterMixin, UpdateView):
    ...
    def get_url_list(self):
        if self.is_popup() and self.object is not None:
            app_label, module_name = self.object.model.split(".")
            return self.wrap_url_popup(
                reverse("dwfilter.webixfilter.list_model", kwargs={"app_label": app_label, "model_name": module_name})
            )
        else:
            return super().get_url_list()
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({"check_filter": correct, "limit_suggest": get_limit_suggest()})
        return context
