...
def get_JSON_for_JQB(filtro, model_class=None):
    index = 0
    rules_result = {}
    rules_dependecy = {}
    ...
    rules_result = {"rules": filtro, "fks": rules_dependecy}
    return rules_result
    ...
    if filtro is not None and filtro.get("condition") is not None and filtro.get("rules"):
        ...
        for rule in filtro.get("rules", []):
            if rule.get("condition") is not None:
                ...
                entry = {"path": rule.get("id", "") + "__" + rule.get("operator", ""), "val": rule.get("value")}
                qsets.append(entry)
        result.update({"qsets": qsets})
...
def get_JSON_from_DB(filtro):
    ...
    if filtro is not None and filtro.get("operator") is not None and filtro.get("qsets"):
        result.update({"condition": filtro.get("operator", "AND"), "not": filtro.get("negate", True), "valid": True})
        rules = []
...
def tree_visit(rules, d, index, model_class):
    ...
    for rule in rules:
        if rule.get("condition") is not None:
            ...
            model_field = list(filter(lambda f: f.name == field_base, model_class._meta.get_fields()))
            if len(model_field) > 1:
                raise Exception("Error found a repeating field ?")
            model_field = model_field[0]
            next_model = None
            if (
                issubclass(type(model_field), models.ForeignKey)
                or issubclass(type(model_field), ForeignObjectRel)
                or issubclass(type(model_field), models.ManyToManyField)
                or issubclass(type(model_field), GenericRelation)
            ):
            ...
            for field in fields:
                model_field = list(filter(lambda f: f.name == field, next_model._meta.get_fields()))
                if len(model_field) > 1:
                    raise Exception("Error found a repeating field ?")
