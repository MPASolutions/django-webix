...
class GetMessageUnreadView(WebixTemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if message:
            MessageUserRead.objects.create(user=self.request.user, message_sent=message.message_sent)
...
class SenderSendView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for send_method in send_methods.split(","):
            ...
            results.append({"send_method": send_method, "result": result, "status": status})
...
class SenderMessagesListView(WebixListView):
            ...
            },
            {
                ...
                "datalist_column": format_lazy(
                    """{{
                        ...
                        options: message_sent__typology__typology_options}}],
                        adjust: "all"
                    }}""",
                    escapejs(_("Typology")),
                ),
    ...
    def get_url_list(self):
        if self.kwargs.get("typologiesgroup_pk") is not None:
            return reverse(
                "dwsender.messages_list.typologiesgroup",
                kwargs={"typologiesgroup_pk": self.kwargs.get("typologiesgroup_pk")},
            )
        elif self.kwargs.get("typology_pk") is not None:
            return reverse("dwsender.messages_list.typology", kwargs={"typology_pk": self.kwargs.get("typology_pk")})
        else:
        ...
        if pk_attachment is not None:
            ...
            if attachment is not None and hasattr(attachment, model_class.get_file_fieldpath()):
                ...
                if file_field.storage.exists(file_field.name):
                    is_pdf = False
                    if (
                        file_field.name.split(".")[-1] == "pdf"
                        and getattr(settings, "WEBIX_LICENSE", None) == "PRO"
                        and ("pdf_preview" not in CONF["attachments"] or CONF["attachments"]["pdf_preview"])
                    ):
                        is_pdf = True
                    return JsonResponse(
                        {
                            ...
                            "is_pdf": is_pdf,
                            "name": file_field.name.split("/")[-1].split(".")[0],
                        },
                        safe=False,
                    )
...
class SenderMessagesChatView(WebixTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        methods_show = [
            "{}.{}".format(i["method"], i["function"])
            for i in filter(lambda x: x["show_in_chat"] is True, CONF["send_methods"])
        ]
        ...
        if section == "users":
                ...
                recipient_queryset = (
                    recipient_queryset.annotate(
                        ...
                        last_message=Subquery(
                            ...
                            .order_by("-message_sent__creation_date")
                            .values("message_sent__creation_date")[:1]
                        ),
                    )
            ...
            section == "messages" and contenttype is not None and recipient_pk is not None and send_method is not None
        ):
                ...
                messages = (
                    MessageSent.objects.filter(
                        ...
                        pk__in=recipient.message_recipients.filter(
                            Q(message_sent__send_method=send_method) & Q(message_sent__send_method__in=methods_show),
...
class SenderInvoiceManagementView(WebixTemplateView):
    ...
    groups = {
        ...
        "bimestrial": [_("Jan - Feb"), _("Mar - Apr"), _("May - Jun"), _("Jul - Aug"), _("Sep - Oct"), _("Nov - Dec")],
        "quarter": [_("First quarter"), _("Second quarter"), _("Third quarter"), _("Fourth quarter")],
        "half-yearly": [_("First semester"), _("Second semester")],
        "yearly": [_("Year")],
    }
    ...
    def get_context_data(self, **kwargs):
        ...
        senders = senders.distinct("sender", "send_method").values_list("sender", "send_method")
        ...
        for idx, (sender, send_method) in enumerate(senders):
            ...
            _sender = {
                "name": "{}".format(sender),
                "send_method": "{}".format(
                    _send_methods[send_method] if send_method in _send_methods else send_method
                ),
            ...
            for index, period in enumerate(_periods):
                ...
                totals = qs.filter(_filter).aggregate(
                    messages_success=Sum(
                        Case(
                            When(messagerecipient__status="success", then=F("messagerecipient__sent_number")),
                            default=0,
                            output_field=IntegerField(),
                        )
                    ),
                    messages_unknown=Sum(
                        Case(
                            When(messagerecipient__status="unknown", then=F("messagerecipient__sent_number")),
                            default=0,
                            output_field=IntegerField(),
                        )
                    ),
                    messages_fail=Sum(
                        Case(
                            When(messagerecipient__status="failed", then=F("messagerecipient__sent_number")),
                            default=0,
                            output_field=IntegerField(),
                        )
                    ),
                    messages_invoiced=Sum(
                        Case(
                            When(
                                invoiced=True,
...
class SenderTelegramWebhookView(View):
    def post(self, request, *args, **kwargs):
        ...
        # Change language
        if "message" in response and "from" in response["message"] and "language_code" in response["message"]["from"]:
        ...
        for recipient in CONF["recipients"]:
            ...
            model_class = apps.get_model(app_label=app_label, model_name=model)
            telegram_id = None
            if "message" in response and "from" in response["message"] and "id" in response["message"]["from"]:
                telegram_id = response["message"]["from"]["id"]
            elif (
                "my_chat_member" in response
                and "from" in response["my_chat_member"]
                and "id" in response["my_chat_member"]["from"]
            ):
                telegram_id = response["my_chat_member"]["from"]["id"]
            if telegram_id is not None and model_class.get_telegram_fieldpath():
                recipients += list(model_class.objects.filter(**{model_class.get_telegram_fieldpath(): telegram_id}))

        # Save only text messages (exclude commands)
        if (
            "message" in response
            and "from" in response["message"]
            and "text" in response["message"]
            and not response["message"]["text"].startswith("/")
        ):
            # Create db record
