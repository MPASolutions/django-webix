...
def get_messages_read_required(request, queryset=None):
    ...
    queryset = queryset.filter(
        Q(message_sent__send_method__in=send_methods_read_required)
...
def send_mixin(
    ...
    **kwargs
    ...
    :param send_method: <skebby|email|telegram|storage>.<function>
    (eg. "skebby.django_webix.contrib.sender.send_methods.email.send_utils")
    ...
    message_sent = MessageSent(
        send_method=send_method, subject=subject, body=body, cost=_cost, user=user, sender=_sender
    )
