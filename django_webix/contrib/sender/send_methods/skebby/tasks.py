        ...
        if message_sent.messagerecipient_set.filter(status="unknown").exists():
            ...
            for recipient in response["recipients"]:
                _recipients = message_sent.messagerecipient_set.filter(
                    recipient_address=recipient["destination"], status="unknown"
                )
...
def check_state_history(same_sender_name=True):
    ...
    try:
        ...
        recipients = (
            ...
            .values_list("recipient_address", flat=True)
            .order_by()
            .distinct()
        )
        ...
        for recipient in recipients:
            ...
            results = skebby.sms_history.get_sent_sms_to_recipient(
                recipient=recipient, date_from=date_from.strftime("%Y%m%d000000"), page_size=1000
            )
            ...
            for communication in results["rcpthistory"]:
                ...
                if communication["status"] == 4:
                    message_recipient.update(extra=communication, status="success")
                elif communication["status"] in [9, 10]:
                    message_recipient.update(extra=communication, status="failed")
    ...
    return {
        "remaining": MessageRecipient.objects.filter(
            message_sent__send_method="skebby.django_webix.contrib.sender.send_methods.skebby.send", status="unknown"
        )
        .only(MessageRecipient._meta.pk.name)
        .count()
    }
