...
def check_phone_number(update: Update, context: CallbackContext) -> int:
    ...
    if not _found:
        update.message.reply_text(
            _("Your number is not in our database, contact us to add"), reply_markup=ReplyKeyboardRemove()
        )
...
def cancel_phone_number(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        _("Okay, if you want to connect your account type /start"), reply_markup=ReplyKeyboardRemove()
    )
    return ConversationHandler.END
