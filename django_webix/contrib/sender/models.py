...
class DjangoWebixSender(Model):
    ...
    message_recipients = GenericRelation(
        "dwsender.MessageRecipient", related_query_name="%(class)s_message_recipients"
    )
...
class Customer(DjangoWebixSender):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, blank=True, null=True, on_delete=models.CASCADE, verbose_name=_("User")
    )
    ...
    typology = models.ForeignKey(
        CustomerTypology, blank=True, null=True, on_delete=models.CASCADE, verbose_name=_("Typology")
    )
    creation_date = models.DateTimeField(auto_now_add=True, verbose_name=_("Creation date"))
    modification_date = models.DateTimeField(auto_now=True, verbose_name=_("Modification data"))
...
class ExternalSubject(DjangoWebixSender):
    ...
    extra = JSONField(blank=True, null=True, verbose_name=_("Extra"))
    typology = models.ForeignKey(
        ExternalSubjectTypology, blank=True, null=True, on_delete=models.CASCADE, verbose_name=_("Typology")
    )
    creation_date = models.DateTimeField(auto_now_add=True, verbose_name=_("Creation date"))
    modification_date = models.DateTimeField(auto_now=True, verbose_name=_("Modification data"))
...
class MessageSent(Model):
    ...
    typology = models.ForeignKey(
        MessageTypology, blank=True, null=True, on_delete=models.SET_NULL, verbose_name=_("Typology")
    )
    ...
    attachments = models.ManyToManyField(
        CONF["attachments"]["model"], blank=True, db_constraint=False, verbose_name=_("Attachments")
    )
    read_required = models.BooleanField(_("Read required"), blank=True, null=False, default=False)
...
class MessageRecipient(Model):
    ...
    message_sent = models.ForeignKey(MessageSent, on_delete=models.CASCADE, verbose_name=_("Message sent"))
...
class TelegramPersistence(Model):
    ...
    typology = models.CharField(
        max_length=32, choices=[(i, i) for i in ["user_data", "chat_data", "bot_data", "conversations"]], unique=True
    )
    data = JSONField()
