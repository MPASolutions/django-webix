...
from django_webix.contrib.sender.models import MessageRecipient, MessageSent, MessageTypology, MessageUserRead

CONF = getattr(settings, "WEBIX_SENDER", None)

if CONF is not None and any(
    _recipients["model"] == "django_webix.contrib.sender.Customer" for _recipients in CONF["recipients"]
):
...
if CONF is not None and any(
    _recipients["model"] == "django_webix.contrib.sender.ExternalSubject" for _recipients in CONF["recipients"]
):
...
class MessageSentAdmin(admin.ModelAdmin):
    ...
    list_filter = ("send_method", "user", "cost", "invoiced", "sender", "status", "creation_date", "modification_date")

    def get_queryset(self, request):
        qs = super(MessageSentAdmin, self).get_queryset(request)
        return qs.annotate(
            recipients_count=Count("messagerecipient"),
            recipients_success=Sum(
                Case(When(messagerecipient__status="success", then=1), default=0, output_field=IntegerField())
            ),
            recipients_failed=Sum(
                Case(When(messagerecipient__status="failed", then=1), default=0, output_field=IntegerField())
            ),
            recipients_unknown=Sum(
                Case(When(messagerecipient__status="unknown", then=1), default=0, output_field=IntegerField())
            ),
            recipients_invalid=Sum(
                Case(When(messagerecipient__status="invalid", then=1), default=0, output_field=IntegerField())
            ),
            recipients_duplicate=Sum(
                Case(When(messagerecipient__status="duplicate", then=1), default=0, output_field=IntegerField())
            ),
        )
