        ...
        for send_method in self.CONF["send_methods"]:
            ...
            if "recipients_clean" not in send_method:
                if send_method["method"] == "email":
                    ...
                    send_method["recipients_clean"] = (
                        "django_webix.contrib.sender.send_methods.skebby.recipients_clean"
                    )
                elif send_method["method"] == "telegram":
                    send_method["recipients_clean"] = (
                        "django_webix.contrib.sender.send_methods.telegram.recipients_clean"
                    )
                elif send_method["method"] == "storage":
                    send_method["recipients_clean"] = (
                        "django_webix.contrib.sender.send_methods.storage.recipients_clean"
                    )
                else:
            ...
            if "attachments_format" not in send_method:
                if send_method["method"] == "email":
                    send_method["attachments_format"] = (
                        "django_webix.contrib.sender.send_methods.email.attachments_format"
                    )
                elif send_method["method"] == "skebby":
                    send_method["attachments_format"] = (
                        "django_webix.contrib.sender.send_methods.skebby.attachments_format"
                    )
                elif send_method["method"] == "telegram":
                    send_method["attachments_format"] = (
                        "django_webix.contrib.sender.send_methods.telegram.attachments_format"
                    )
                elif send_method["method"] == "storage":
                    send_method["attachments_format"] = (
                        "django_webix.contrib.sender.send_methods.storage.attachments_format"
                    )
                else:
    ...
    def recipients_check(self):
        ...
        for recipient in self.CONF["recipients"]:
            ...
            if "datatable_fields" not in recipient or not isinstance(recipient["datatable_fields"], list):
                raise ImproperlyConfigured(
                    _("You have to define a `datatable_fields` as list in your recipients list")
                )
