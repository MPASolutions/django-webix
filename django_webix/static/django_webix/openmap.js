...
webix.protoUI({
    ...
    _initMap: function (define) {
        ...
        L.drawLocal.draw.toolbar.buttons.polygon = 'Disegna una geometria';
        L.drawLocal.edit.toolbar.buttons.edit = 'Modifica geometria';
        L.drawLocal.edit.toolbar.buttons.remove = 'Cancella geometria';
        ...
        if (c.saveUrl) {
            ...
            map.on('draw:created', function (event) {
                ...
                $.ajax({
                    ...
                    method: "POST",
                    data: {'geo':JSON.stringify(editableLayer.toGeoJSON())},
                    success: function (data) {
                        if (data.status) {
                            webix.message("La geometria e' stata creata");
                            if (c.postEdit){
                                c.postEdit(editableLayer)
                            }
                        } else {
                            webix.message("Non e' possibile creare la geometria");
                        }
                    }
                });
            });
            ...
            map.on('draw:edited', function (event)  {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status) {
                            webix.message("La geometria e' stata aggiornata");
                            if (c.postEdit){
                                c.postEdit(editableLayer)
                            }
                        } else {
                            webix.message("Non e' possibile aggiornare la geometria");
                        }
                    }
                });
              });
            ...
            map.on('draw:deleted', function (event) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status) {
                            webix.message("La geometria e' stata rimossa");
                            if (c.postEdit){
                                c.postEdit(editableLayer)
                            }
                        } else {
                            webix.message("Non e' possibile rimuovere la geometria");
                        }
                    }
                });
              });
