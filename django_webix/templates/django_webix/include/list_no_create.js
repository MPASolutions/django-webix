        ...
        $$("{{ view.webix_view_id|default:"content_right" }}").addView({
            cols:[
                {$template: "Spacer"},
                {
                    ...
                    height: 50,
                    view: "button",
                    id: 'button_add_{{ forloop.counter }}',
                    type: "htmlbutton",
                    label: "<span style='font-size:22px;'>Aggiungi {{ el|getattr:"_meta"|getattr:"verbose_name" }}</span>"
                },
                {$template: "Spacer"}
            ]
        });
        {% if el.get_url_list and el.get_url_list != '' %}
